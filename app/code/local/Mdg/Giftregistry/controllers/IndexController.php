...
class Mdg_Giftregistry_IndexController extends Mage_Core_Controller_Front_Action {
	...
	public function deleteAction(){
		try{
			...
			if($registryId && $this->getResponse()->getPost()){
		...
		try{
			...
			if($this->getRequest()->getPost() && $empty($data){
				...
				if($registry){
					$registry->updateRegistryData($customer, $data);
					$registry.save();
					$successMessage = Mage::helper('mdg_giftregistry')->__('Registry Successfully Updated');
					Mage::getSingleton('customer/session')->addSuccess($successMessage);
				} else {
			...
			Mage::getSingleton('customer/session')->addError($e->getMesage());
			$this->_redirect('*/*/');
		}
		$this->_redirect('*/*/');
	}
